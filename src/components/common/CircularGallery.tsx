...
class Title {
  ...
  constructor({ gl, plane, renderer, text, textColor = '#399139', font = 'bold 28px sans-serif' }: TitleProps) {
    autoBind(this);
    this.gl = gl;
    this.plane = plane;
...
class Media {
  ...
  onResize({ screen, viewport }: { screen?: ScreenSize; viewport?: Viewport } = {}) {
    ...
    this.plane.scale.x = (this.viewport.width * ((isMobile ? 850 : 700) * this.scale)) / this.screen.width;
...
export class CircularGalleryApp {
  ...
  startX: number = 0;
  startY: number = 0;
  isHorizontalGesture: boolean = false;
  ...
  createMedias(
    ...
    const defaultItems = [
      { image: `https://picsum.photos/seed/1/800/600`, text: 'Biodegradable' },
      { image: `https://picsum.photos/seed/2/800/600`, text: 'Compostable' },
      { image: `https://picsum.photos/seed/3/800/600`, text: 'Plastic Free' },
      { image: `https://picsum.photos/seed/4/800/600`, text: 'Food-Grade Board' },
      { image: `https://picsum.photos/seed/5/800/600`, text: 'Recyclable' },
      { image: `https://picsum.photos/seed/6/800/600`, text: 'Oil Resistant' }
    ];
  ...
  onTouchDown(e: MouseEvent | TouchEvent) {
    ...
    this.isHorizontalGesture = !('touches' in e);
  }
  ...
  onTouchMove(e: MouseEvent | TouchEvent) {
    ...
    if ('touches' in e && !this.isHorizontalGesture) {
      const dx = Math.abs(currentX - this.startX);
      const dy = Math.abs(currentY - this.startY);
      if (dx > dy && dx > 6) {
        this.isHorizontalGesture = true;
      } else if (dy > dx && dy > 8) {
        // Vertical page scroll detected -> release touch to allow natural page scroll
        this.isDown = false;
        return;
      }
    }

    if (this.isHorizontalGesture || !('touches' in e)) {
  ...
  onResize() {
    ...
    this.screen = {
      ...
      height: this.container.clientHeight || 550
    };
  ...
  addEventListeners() {
    ...
    // Attach interaction listeners scoped to container for smooth, non-intrusive navigation
...
export const CircularGallery = forwardRef<CircularGalleryRef, CircularGalleryProps>(
  (
    ...
    useEffect(() => {
      ...
      let app: CircularGalleryApp | undefined;
      let isMounted = true;
